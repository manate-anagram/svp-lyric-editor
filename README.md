...
- **音声クリップ配置** — `.wav` / `.mp3` 等をオーディオ帯に配置（最大1本）してドラッグ移動/長さ調整/**ミュート・ソロ**、プレビュー再生で試聴（配置・移動・削除は Undo 対応）。時間軸とスナップはピアノロールと共通、全体ビューにも音声レーンを表示。音声はセッション内のみで、svpには書き出しません（既存の音声トラックは保持）svp書き出し時は参照（`mainRef.audio.filename`・ファイル名のみ）を含め、音声をsvpと同じ場所に置けばSynthVで復元される（SV Studioのオーディオトラックと同じ外部参照方式・実体ファイルは埋め込まれない）
- **スマホ対応** — 狭い画面ではトラックペインを☰ボタンのドロワー化（トラック/音声のM/S・削除も操作可、選択で自動クローズ）し上部セレクタも併設、**歌詞ストリップ優先**レイアウト。タップ=歌詞編集/シーク、1本指スワイプ=スクロール、ピンチ=ズーム（Ctrl+ホイール相当）。歌詞ストリップで選んだノートはメイン表示域に自動スクロール
