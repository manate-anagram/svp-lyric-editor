...
- **音声クリップ配置** — `.wav` / `.mp3` 等をオーディオ帯に配置（複数はレーン分け）してドラッグ移動/長さ調整/**ミュート・ソロ**、プレビュー再生で試聴（配置・移動・削除は Undo 対応）。時間軸とスナップはピアノロールと共通、全体ビューにも音声レーンを表示svp書き出し時は参照（`mainRef.audio.filename`・ファイル名のみ）を含め、音声をsvpと同じ場所に置けばSynthVで復元される（SV Studioのオーディオトラックと同じ外部参照方式・実体ファイルは埋め込まれない）
